...
class TestCreditCardForm:
    ...
    @patch("monopyly.credit.forms.CreditCardForm.AccountSubform.gather_entry_data")
...
class TestCreditTransactionForm:
    ...
    def test_statement_subform_get_statement(
        self, mock_method, filled_transaction_form
    ):
    ...
    def test_statement_subform_get_statement_no_card(
        self, mock_method, filled_transaction_form
    ):
    ...
    def test_statement_subform_determine_inferred_statement(
        ...
        statement = statement_subform.determine_statement(mock_card, transaction_date)
        assert statement == mock_method.return_value
        mock_method.assert_called_with(mock_card, transaction_date, creation=True)
    ...
    def test_statement_subform_gather_card_data(
        self, mock_method, transaction_form, mock_card
    ):
        statement_subform = transaction_form.statement_info
        data = statement_subform.gather_entry_data(mock_card)
        expected_data = {"card_info": mock_method.return_value}
        assert data == expected_data
    ...
    @patch("monopyly.credit.forms.CreditTransactionForm.StatementSubform.get_statement")
