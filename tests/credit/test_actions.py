...
from monopyly.banking.transactions import BankTransactionRepository
...
from monopyly.credit.transactions import CreditTransactionRepository


@patch("monopyly.credit.actions.CreditStatementRepository.get_statements")
...
def _add_extra_active_card():
    ...
    return CreditCardRepository.add_entry(
        account_id=3,
...
def test_get_potential_preceding_card_no_statements(client_context):
    ...
    CreditCardRepository.update_entry(4, active=0)
    # Mock the card to be tested for a preceding card
    card = Mock(id=6, active=1, account_id=3)
    preceding_card = get_potential_preceding_card(card)
    assert preceding_card is None
...
def test_get_potential_preceding_card_no_balance(app, client_context):
    ...
    statement = CreditStatementRepository.add_statement(
        other_active_card, date(2020, 7, 10), due_date=date(2020, 8, 3)
    )
    CreditTransactionRepository.add_entry(
...
def test_transfer_credit_card_statement(client_context, transfer):
    ...
    card = CreditCardRepository.add_entry(
    ...
    latest_statement = CreditStatementRepository.get_entry(5)
    ...
    prior_card = CreditCardRepository.get_entry(prior_card_id)
...
def test_make_payment(client_context):
    ...
    payment_bank_transaction = BankTransactionRepository.get_entry(8)
    ...
    payment_credit_transaction = CreditTransactionRepository.get_entry(14)
