...
class NearMatchFinder(MatchFinder):
    ...
    is comparable to the recorded transaction total).
    """
    ...
    def is_match(cls, transaction, activity):
        ...
        return near_date and near_amount

    @classmethod
    def _is_near_date(cls, transaction, activity):
        low_date = activity.transaction_date - timedelta(days=1)
        high_date = activity.transaction_date + timedelta(days=1)
...
class _Matchmaker(ABC):
    ...
    def _choose_best_ambiguous_match(self, transaction, activities):
        ...
        merchant_tokens = self._tokenize(transaction.merchant)
        notes_tokens = self._tokenize(transaction.notes)
        score_records = []
        for activity in activities:
            activity_tokens = self._tokenize(activity.description)
            score = self._compute_transaction_activity_similarity_score(
                merchant_tokens, notes_tokens, activity_tokens
            )
            score_records.append((activity, score))
        # The activity with the lowest score is chosen as the best match
        best_match = min(score_records, key=lambda record: record[1])[0]
        return best_match
    ...
    def _compute_transaction_activity_similarity_score(
        ...
        """
        merchant_score = self._score_tokens(merchant_tokens, activity_tokens)
        notes_score = self._score_tokens(notes_tokens, activity_tokens)
        return merchant_score, notes_score

    @staticmethod
    def _score_tokens(reference, test):
